#!/bin/bash
# =============================================================================
# Full End-to-End Workshop Smoke Test — LangGraph Track (Modules 02-10)
# =============================================================================
...
#
# Usage:
#   ./test_langgraph_full.sh [username]
#   username defaults to rroe@example.com
#
# You can also start from a specific phase:
#   ./test_langgraph_full.sh [username] --skip-local     # Skip Phase 1
#   ./test_langgraph_full.sh [username] --phase2-only     # Only Phase 2
#   ./test_langgraph_full.sh [username] --phase3-only     # Only Phase 3 (requires Phase 2 done)
# =============================================================================
...
LOGFILE="test_langgraph_full_$(date +%Y%m%d_%H%M%S).log"
PASS=0
FAIL=0
SKIP=0
FRAMEWORK="langgraph"
...
if [ "$SKIP_LOCAL" = "true" ]; then
    ...
    for f in modules/02/langgraph/server.py \
             modules/02/langgraph/agent.py \
             modules/03/langgraph/dataviz.py \
             modules/04/langgraph/orchestrator_agent.py; do
        if [ -f "$f.backup" ]; then
    ...
    # ── Step 2: MCP Server ────────────────────────────────────────────────
    log ""; log "  --- MCP Server test (modules/02/langgraph) ---"

    OUTPUT=$(run_with_timeout 30 uv run modules/02/langgraph/test_server.py "$USERNAME" 2>&1)
    echo "$OUTPUT" >> "$LOGFILE"
    ...
    log ""; log "  --- DataViz Agent (modules/03/langgraph) ---"

    OUTPUT=$(printf '"Month,Sales\nJan,1000\nFeb,1200\nMar,800\nApr,1500"\n\nCreate a bar chart showing sales by month\nEND\nquit\n' | \
        run_with_timeout 120 uv run modules/03/langgraph/dataviz.py \
        -m "$MODEL" 2>&1)
    echo "$OUTPUT" >> "$LOGFILE"
    ...
    log ""; log "  --- Orchestrator Agent (modules/04/langgraph) ---"

    OUTPUT=$(run_with_timeout 30 uv run modules/04/langgraph/test_orchestrator.py 2>&1)
    echo "$OUTPUT" >> "$LOGFILE"
    [ $? -eq 0 ] && pass "Orchestrator import test" || fail "Orchestrator import test"

    OUTPUT=$(printf 'What rate plans are available?\nquit\n' | \
        run_with_timeout 180 uv run modules/04/langgraph/orchestrator_agent.py \
        -m "$MODEL" \
        -u "$USERNAME" \
        -t "smoketest-langgraph-orch-$$" 2>&1)
    echo "$OUTPUT" >> "$LOGFILE"
...
if [ "$PHASE3_ONLY" = "true" ]; then
    ...
    OUTPUT=$(run_with_timeout 300 uv run modules/05/langgraph/deploy_lambda.py \
        --server-name electrify-server \
        --db-cluster-arn "$PGHOSTARN" \
        --secret-arn "$PGSECRET" \
        --database "$PGDATABASE" \
        --mcp-server-path modules/05/langgraph/electrify_server.py \
    ...
    OUTPUT=$(run_with_timeout 300 uv run modules/05/langgraph/deploy_lambda.py \
        ...
        --mcp-server-path modules/05/langgraph/dataviz_server.py \
    ...
    if [ -n "$ELECTRIFY_LAMBDA_ARN" ] && [ -n "$DATAVIZ_LAMBDA_ARN" ]; then
        OUTPUT=$(run_with_timeout 600 uv run modules/05/langgraph/deploy_gateway_simple.py \
    ...
    if [ -n "$MCP_GATEWAY_URL" ] && [ -n "$TOKEN" ]; then
        OUTPUT=$(run_with_timeout 60 uv run modules/05/langgraph/test_gateway_auth.py \
    ...
    if [ -n "$MCP_GATEWAY_URL" ]; then
        cd ~/workshop/modules/05/langgraph 2>/dev/null || cd modules/05/langgraph
    ...
    if [ -n "$AGENT_RUNTIME_ID" ] && [ -n "$TOKEN" ]; then
        ...
        OUTPUT=$(run_with_timeout 120 uv run modules/05/langgraph/test_runtime.py \
