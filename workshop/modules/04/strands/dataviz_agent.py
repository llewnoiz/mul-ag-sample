...
Strands equivalent of the LangGraph dataviz_agent.py.
Accepts datasets and explanations to generate appropriate data visualizations
using matplotlib and returns base64-encoded PNG chart images.

Usage as a module:
    from dataviz_agent import DataVizAgent, create_dataviz_agent
...
Usage as CLI:
    python modules/04/strands/dataviz_agent.py -m global.anthropic.claude-sonnet-4-20250514-v1:0
"""
...
class MCPConfigLoader:
    ...
    def parse_servers(self, config: Dict[str, Any]) -> List[MCPClient]:
        """Parse server configurations into MCPClient instances."""
        if 'mcp_servers' not in config:
            return []
        clients = []
